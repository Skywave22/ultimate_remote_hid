...
class LayoutNotifier extends StateNotifier<LayoutState> {
  LayoutNotifier() : super(LayoutState(
    ...
    themePrimaryColor: const Color(0xFF2196F3),
  ));
  ...
  static List<AppComponent> _generateDefaultLayout() {
    return [
      AppComponent(
        ...
        relativePosition: const Offset(0.05, 0.2),
        relativeSize: const Size(0.5, 0.6),
      ...
      AppComponent(
        ...
        relativePosition: const Offset(0.6, 0.2),
        relativeSize: const Size(0.15, 0.15),
        color: Colors.blue,
        actionValue: 'ENTER',
      ),
      AppComponent(
        ...
        label: 'SPACE',
        type: ComponentType.button,
        relativePosition: const Offset(0.6, 0.4),
        relativeSize: const Size(0.3, 0.12),
        color: Colors.indigo,
        actionValue: 'SPACE',
      ),
      AppComponent(
        ...
        relativePosition: const Offset(0.08, 0.82),
        relativeSize: const Size(0.22, 0.15),
        color: Colors.teal,
        actionValue: 'L_STICK',
      ),
    ];
      ...
      if (c.id == id) {
        return c.copyWith(
          relativePosition: Offset(
            (c.relativePosition.dx + delta.dx / screenSize.width).clamp(0.0, 0.9),
            (c.relativePosition.dy + delta.dy / screenSize.height).clamp(0.0, 0.9),
          ),
        );
      }
      return c;
  ...
  void updateComponentProperty(String id, Map<String, dynamic> updates) {
    final updated = state.components.map((c) {
      if (c.id == id) {
        return c.copyWith(
          label: updates['label'],
          relativeSize: updates['size'],
          color: updates['color'],
          actionValue: updates['action'],
        );
      }
      return c;
    }).toList();
  ...
  void loadPreset(List<AppComponent> preset) {
    state = state.copyWith(components: preset);
  }
}
